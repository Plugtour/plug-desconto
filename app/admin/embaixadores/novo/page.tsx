...
// app/admin/embaixadores/novo/page.tsx
...
export default function AdminEmbaixadoresNovoPage() {
  const router = useRouter();
  const { showToast } = useAdminToast();
  const { createAmbassador } = useAdminData();
  ...
  const [status, setStatus] = useState<AmbassadorStatus>('rascunho');

  const canSave = useMemo(() => {
    return nome.trim().length >= 2 && email.trim().length >= 3;
  }, [nome, email]);
    ...
    <main className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Novo embaixador</h1>
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Cadastro rápido com status e código.</p>
          </div>
        </div>
      ...
      <div className="grid gap-4 rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <div className="text-[11px] text-zinc-500 dark:text-zinc-500">
            Salvo apenas com números: {onlyDigits(whatsapp)}
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          <div className="text-[11px] text-zinc-500 dark:text-zinc-500">
            Fica assim: {slugifyCode(codigo || nome) || '—'}
          </div>
        </div>
