...
export default function AdminDestinosClient() {
  ...
  async function onUpdate(id: string, patch: Partial<Pick<Destino, 'nome' | 'ativo'>>) {
    setErr(null);
    try {
      const r = await fetch(`/api/admin/config/destinos/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function onDelete(id: string) {
    ...
    setErr(null);
    try {
      const r = await fetch(`/api/admin/config/destinos/${id}`, { method: 'DELETE' });
      const j = await r.json();
      if (!r.ok) throw new Error(j?.error || 'Erro');
      await refresh();
    } catch (e: any) {
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-300">
            Cadastre cidades/regiões. Eles serão usados nos formulários e no site.
          </p>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="text-sm font-medium">Novo destino</div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
          {items.map((d) => (
            <Row key={d.id} item={d} onUpdate={onUpdate} onDelete={onDelete} />
          ))}
          {items.length === 0 && <div className="px-4 py-6 text-sm text-zinc-500">Nenhum destino cadastrado.</div>}
        </div>
      </div>
    </div>
  );
...
function Row({
  ...
  item: Destino;
  onUpdate: (id: string, patch: Partial<Pick<Destino, 'nome' | 'ativo'>>) => void;
