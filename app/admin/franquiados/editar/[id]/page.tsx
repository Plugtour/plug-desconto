...
// app/admin/franquiados/editar/[id]/page.tsx
...
export default function AdminFranquiadosEditarPage() {
  const router = useRouter();
  const params = useParams<{ id: string }>();
  const id = String(params?.id ?? '');

  const { showToast } = useAdminToast();
  const { franchisees, updateFranchisee } = useAdminData();

  const current = useMemo(() => {
    return franchisees.find((x) => x.id === id) ?? null;
  }, [franchisees, id]);

  const [nome, setNome] = useState(current?.nome ?? '');
  const [cidade, setCidade] = useState(current?.cidade ?? '');
  const [whatsapp, setWhatsapp] = useState(current?.whatsapp ?? '');
  const [imageUrl, setImageUrl] = useState(current?.imageUrl ?? '');
  const [status, setStatus] = useState<FranchiseeStatus>((current?.status ?? 'rascunho') as FranchiseeStatus);
  ...
  const onSave = () => {
    if (!current) {
      showToast('Franquiado não encontrado', 'error');
      return;
    }
    if (!canSave) {
    ...
    const updated = updateFranchisee({
      id,
      nome: nome.trim(),
      cidade: cidade.trim(),
    ...
    return (
      <main className="space-y-4">
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
          <div className="text-sm font-medium">Franquiado não encontrado</div>
          <div className="mt-2">
            <Link className="text-sm underline" href="/admin/franquiados">
              Voltar
            </Link>
          </div>
        </div>
    ...
    <main className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Editar franquiado</h1>
